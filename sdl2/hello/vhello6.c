...
int main() {
	...
	int lame_x,lame_y;
	int lame_dragging=0;
	int lame_draggable=0;
	int lame_drag_ox=0,lame_drag_oy=0;
	...
	while (1) {
		if (SDL_WaitEventTimeout(&event,1000 / demo_fps)) {
			...
			/* allow user to drag away the "lame" graphic */
			else if (event.type == SDL_MOUSEBUTTONDOWN) {
				if (event.button.button == SDL_BUTTON_LEFT) {
					if (!lame_dragging && lame_draggable) {
						if (event.button.x >= lame_x && event.button.x < (lame_x+lame_w) &&
							event.button.y >= lame_y && event.button.y < (lame_y+lame_h)) {
							lame_drag_ox = event.button.x - lame_x;
							lame_drag_oy = event.button.y - lame_y;
							SDL_SetWindowGrab(main_wnd,1);
							lame_dragging = 1;
							redraw = 1;
						}
					}
				...
				if (lame_dragging) {
					lame_x = event.motion.x - lame_drag_ox;
					lame_y = event.motion.y - lame_drag_oy;
					redraw = 1;
				}
			}
		}
